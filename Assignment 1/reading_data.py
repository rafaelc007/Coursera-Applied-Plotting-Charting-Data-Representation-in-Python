...
def run_ttest():
    ...
    # calculate ratios
    r1 = (data_rec_uni/data_bec_uni)
    r2 = (data_rec_house/data_bec_house)
    
    # run t-test
    test = stats.ttest_ind(r1.dropna(), r2.dropna(), equal_var = False)
    
    ## calculate minumum mean price ration
    m1 = r1.mean()
    m2 = r2.mean()
    
    ## hypothesis evaluation
    evalut = list()
    for i in range(0,len(test.pvalue)):
        p = test.pvalue[i]
        if m1[i]>m2[i]:
            better = "university town"
        else:
            better = "non-university town"
        evalut.append((p<0.01, p, better))
